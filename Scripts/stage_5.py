...
def setup_data(batch_size=128):
    ...
    # Validação com transform_test (sem augmentations)
...
def train_improved_sota(model, trainloader, valloader, epochs=200):
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'se_resnet18_stage5_best.pth')
            saved_msg = "-> Modelo Guardado!"

        duration = time.time() - epoch_start
        print(f"Epoch {epoch+1}/{epochs} | Time: {duration:.1f}s | LR: {lrs[-1]:.4f} | "
              f"Train Loss (Mix): {t_loss:.4f} | Val Acc: {val_acc:.4f} {saved_msg}")
