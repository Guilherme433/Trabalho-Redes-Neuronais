...
class ResNet18CIFAR(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Adaptação para CIFAR-10: Conv 3x3, stride 1, sem MaxPool inicial
...
def train_sota(model, trainloader, valloader, epochs=200):
...
def main():
    trainloader, valloader, testloader = setup_sota_data(batch_size=128)
    ...
    print(f"Final Test Set Accuracy (Best Model): {final_acc:.4f}")
    ...
    plt.subplot(1, 3, 1)
    plt.plot(epochs_range, train_stats['acc'], label='Train')
    plt.plot(epochs_range, val_stats['acc'], label='Val')
    plt.title('Accuracy')
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.subplot(1, 3, 2)
    plt.plot(epochs_range, train_stats['loss'], label='Train')
    plt.plot(epochs_range, val_stats['loss'], label='Val')
    plt.title('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    final_train_loss = train_stats['loss'][-1]
