...
#Ablation C: SE-ResNet com MixUp mas sem Label Smoothing
...
def train_ablation(model, trainloader, valloader, epochs=200):
    ...
    print(f"Início Ablation C (SE-ResNet | MixUp ON | NO LabelSmooth) por {epochs} epochs...")
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'resnet18_ablation_C_best.pth')
            saved_msg = "-> Modelo Guardado!"

        duration = time.time() - epoch_start
        print(f"Epoch {epoch+1}/{epochs} | Time: {duration:.1f}s | LR: {lrs[-1]:.4f} | "
...
def main():
    ...
    model.load_state_dict(torch.load('resnet18_ablation_C_best.pth'))
    ...
    print(f"\nFinal Test Set Accuracy (Ablation C - No LabelSmooth): {final_acc:.2f}%")
    ...
    plt.title('Accuracy (No Label Smoothing)')
    plt.legend()
    ...
    plt.title('Loss (No Label Smoothing)')
    plt.legend()
    plt.grid(True, alpha=0.3)
